...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!ids.length) {
      return NextResponse.json({ error: "No row ids received" }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      remainingRawCount: rawRows.length,
      summaryCount: summaryRows.length,
      message: "Headcount rows deleted successfully",
    });
  } catch (error: any) {
    return NextResponse.json(
      {
        ok: false,
        error: error?.message || "Failed to delete headcount rows",
      },
      { status: 500 }
    );
  }
